...
def update_last_line(file_path, new_line):
...
class Trainer:
    ...
    def evaluate(self):
        ...
        for batch in self.val_loader:
            ...
            with torch.no_grad():
                outputs = self.model.module(pixel_values=images, return_dict=True)
            ...
            iou = mean_iou(preds, masks, self.model.module.config.num_labels)
            total_iou += iou
            num_batches += 1
    ...
    def train(self):
        for epoch in range(self.epochs):
            ...
            if epoch_iou > self.best_iou:
                ...
                self.model.module.save_pretrained(self.save_dir)
            logging.info("---")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_dir", type=str, default=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "datasets"), help="Path to the dataset directory")
    ...
    parser.add_argument("--log_name", type=str, default="segformer_ddp_training.log", help="Name of the log file")
    args = parser.parse_args()

    world_size = args.n_gpus
    mp.spawn(setup, args=(world_size, args,), nprocs=world_size, join=True)
